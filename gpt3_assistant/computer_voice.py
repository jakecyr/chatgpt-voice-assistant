...
class ComputerVoice:
    ...
    def get_gtts(self, text_to_speak: str):
        logging.debug(f"ComputerVoice.get_gtts - Language: {self._language}, TLD: {self._top_level_domain}")
        ...
        try:
            ...
            if self._language is not None and self._top_level_domain is not None:
                ...
                gtts_instance = gtts.gTTS(text_to_speak, lang=self._language, tld=self._top_level_domain)
            else:
            ...
            print(f"Text to speak, '{text_to_speak}', can not be empty (before or after cleaning): {e}")
        except ValueError as e:
            print(f"Specified lang, '{self._language}', is not supported: {e}")
        except RuntimeError as e:
            print(f"Unable to load language dictionaries for language '{self._language}': {e}")
        except Exception as e:
